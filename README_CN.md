...
WARP 模式支持 Ubuntu/Debian 和 Alpine。Ubuntu/Debian 优先使用 Cloudflare 官方 WARP 客户端；Alpine 使用 `wgcf` 生成 WARP WireGuard 配置，并通过 sing-box 内置 WireGuard endpoint 出口，因为 Cloudflare 没有提供一等支持的 Alpine `apk` 包。
